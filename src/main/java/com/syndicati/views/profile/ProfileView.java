package com.syndicati.views.profile;
...
public class ProfileView implements ViewInterface {
    ...
    private VBox createHeroCard() {
        ...
        Text avatarText = text("S", 38, true, "#ffffff");
        avatarWrap.getChildren().addAll(avatar, avatarText);

        VBox identity = new VBox(10);
        identity.setAlignment(Pos.TOP_LEFT);
        Text name = text("Syndicati Member", 34, true, "#ffffff");
        Text role = text("Resident", 16, false, textSoft());
        Text email = text("contact@syndicati.tn", 14, false, textMuted());
