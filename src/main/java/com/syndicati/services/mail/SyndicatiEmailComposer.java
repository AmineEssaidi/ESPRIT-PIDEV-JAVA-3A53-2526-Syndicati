...
public final class SyndicatiEmailComposer {
    ...
    public static String twoFactorCode(String firstName, String code) {
        String body =
            ...
            "</div>" +
            "<p style='font-size:14px;color:rgba(255,255,255,0.4);text-align:center;margin-top:40px;'>Syndicati Neural Network â€¢ Advanced Identity Protection</p>";

        String footerExtra = "<p style='margin-top:15px;font-style:italic;color:#ff4b5c;font-size:11px;'>Protocol: AUTH-2FA-" + DateTimeFormatter.ofPattern("yyyyMMdd").format(LocalDateTime.now()) + " â€¢ Encrypted Transmission</p>";
    ...
    public static String passwordReset(String firstName, String newPassword) {
        String body =
            ...
            "<p style='font-size:14px;color:rgba(255,255,255,0.4);text-align:center;margin-top:40px;'>Syndicati Security Services â€¢ Threat Mitigation Unit</p>";

        String footerExtra = "<p style='margin-top:15px;font-style:italic;color:#ff4b5c;font-size:11px;'>Ref: PWD-RST-" + DateTimeFormatter.ofPattern("HHmmss").format(LocalDateTime.now()) + " â€¢ Zero Trust Protocol</p>";
        return wrap(body, footerExtra);
    }
    ...
    private static String wrap(String body, String footerExtra) {
        return "<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><style>"
            ...
            + "<p style='letter-spacing:2px;text-transform:uppercase;font-size:10px;margin-top:15px;color:rgba(255,75,92,0.5);'>Obsidian Engine â€¢ Red Protocol</p>"
            + footerExtra
