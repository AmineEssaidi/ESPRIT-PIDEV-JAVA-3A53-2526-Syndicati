...
public class MediaPipeService {
    ...
    private static final String TAG = "MediaPipeService";
    ...
    /**
     * Initialize the MediaPipe Python service
    ...
    public synchronized boolean initialize() {
        ...
        try {
            ...
            if (pythonCmd == null) {
                ...
                log("  Then install: pip install mediapipe opencv-python numpy");
                return false;
            }
            ...
            if (!checkPythonPackages(pythonCmd)) {
                ...
                log("  Run: pip install mediapipe opencv-python numpy");
                return false;
            }
            
            String scriptPath = new File("insightface_service.py").getAbsolutePath();
            if (!new File(scriptPath).exists()) {
                log("ERROR: insightface_service.py not found at: " + scriptPath);
                return false;
            }
            
            log("Starting Python InsightFace service from: " + scriptPath);
            ...
            // Start thread to capture stderr/stdout for debugging
            captureProcessOutput();
            
            threadPool = Executors.newFixedThreadPool(2);
            gson = new GsonBuilder().serializeSpecialFloatingPointValues().create();
            
            // Test connection with ping - MediaPipe takes time to load (~3-5 seconds)
            ...
            log("ERROR: Failed to connect to InsightFace service (timeout after 7.5 seconds)");
            log("  This usually means Python packages are not installed");
            log("  Try running: pip install insightface onnxruntime opencv-python");
            ...
            log("ERROR initializing MediaPipe service: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
        ...
        Thread outputThread = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(
                ...
                while ((line = reader.readLine()) != null) {
                    // Log normal output
                    if (line.contains("INFO") || line.contains("Successfully")) {
                        log(line);
                    }
                }
            } catch (Exception e) {
            ...
            try (BufferedReader errorReader = new BufferedReader(
                ...
                while ((line = errorReader.readLine()) != null) {
                    ...
                    if (line.contains("ModuleNotFoundError: No module named 'insightface'")) {
                        log("ERROR: InsightFace not installed");
                        log("  Run: pip install insightface onnxruntime");
                        suppressLogging = true;
                    } else if (line.contains("ModuleNotFoundError") || line.contains("ImportError")) {
                        log("ERROR: " + line);
                        log("  Missing Python package - run: pip install insightface onnxruntime opencv-python");
                        suppressLogging = true;
                    } else if (line.contains("Traceback") || line.contains("Error")) {
                        System.out.println("[InsightFace-Python] " + line);
                    }
                }
            } catch (Exception e) {
                // Ignore
    ...
    private boolean checkPythonPackages(String pythonCmd) {
        try {
            // Check each package individually for better diagnostics
            String[] packages = {"insightface", "cv2", "numpy"};
            String[] importCmds = {
                "import insightface",
                "import cv2",
                "import numpy"
            ...
            for (int i = 0; i < packages.length; i++) {
                ...
                if (exitCode != 0) {
                    ...
                    if (packages[i].equals("insightface") && !suppressLogging) {
                        log("  ERROR: InsightFace not installed");
                        log("  Install with: pip install insightface onnxruntime opencv-python");
                    }
    ...
    public FaceMesh processFaceFrame(BufferedImage frame, boolean checkSpoofing) {
        ...
        try {
            ...
            if (responseLine == null) {
                log("ERROR: No response from MediaPipe service");
                return null;
            }
            ...
            FaceMesh mesh = gson.fromJson(response, FaceMesh.class);
    ...
    private boolean ping() {
        try {
            ...
            synchronized (processInput) {
                processInput.write(gson.toJson(pingRequest));
                processInput.newLine();
                processInput.flush();
            }
            
            String response = null;
            // Use a timeout for reading response
            long startTime = System.currentTimeMillis();
            while (response == null && System.currentTimeMillis() - startTime < 1000) {
                synchronized (processOutput) {
    ...
    public synchronized void shutdown() {
        if (pythonProcess != null) {
            try {
                ...
                log("MediaPipe service shut down successfully");
            } catch (Exception e) {
