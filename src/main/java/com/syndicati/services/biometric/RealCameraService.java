...
public class RealCameraService {
        ...
        try {
            ...
            if (faceIs != null) {
                ...
                if (faceDetector != null && !faceDetector.empty()) {
                    System.out.println("RealCameraService: ✓ Face detector loaded successfully");
                    landmarkDetectionAvailable = true;
                } else {
                    System.err.println("RealCameraService: ✗ Face detector loaded but empty (OpenCV issue)");
                }
            } else {
            ...
            if (eyeIs != null) {
                ...
                if (eyeDetector != null && !eyeDetector.empty()) {
                    System.out.println("RealCameraService: ✓ Eye detector loaded successfully");
                    landmarkDetectionAvailable = true;
                } else {
                    System.err.println("RealCameraService: ✗ Eye detector loaded but empty (OpenCV issue)");
                }
            } else {
            ...
            if (landmarkDetectionAvailable) {
                System.out.println("RealCameraService: ✓ HAAR Cascade facial landmark detection ready");
            } else {
                System.out.println("RealCameraService: ✗ HAAR Cascade not available - using skin color detection");
            }
            ...
            System.err.println("RealCameraService: ✗ OpenCV classes not available: " + ex.getMessage());
            landmarkDetectionAvailable = false;
        } catch (Exception ex) {
            System.err.println("RealCameraService: ✗ Error initializing landmark detection: " + ex.getClass().getSimpleName() + " - " + ex.getMessage());
            landmarkDetectionAvailable = false;
        }
    }
    ...
    private void drawSpoofingStatus(java.awt.Graphics2D g2d, InsightFaceService.SpoofingAnalysis spoofing, int imageWidth) {
        ...
        if (spoofing.isSpoof) {
            ...
            statusText = String.format("⚠ SPOOF DETECTED (%.0f%%)", spoofing.spoofingScore * 100);
    ...
    public synchronized FaceQuality assessFrameQuality() {
        ...
        if (rand > 0.3) {
            return new FaceQuality(true, "Face detected ✓");
        }
